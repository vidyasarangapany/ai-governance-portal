...
if page == "Overview":
    ...
    with col1:
        ...
        heat_pivot = heat.pivot(index="risk_level", columns="autonomy_level", values="count").fillna(0)
    ...
    if "review_cadence" in df.columns:
        cad = (
            df["review_cadence"].value_counts().reset_index()
            .rename(columns={"index": "review_cadence", "review_cadence": "count"})
        )
        fig3 = px.bar(cad, x="review_cadence", y="count", text="count")
...
# -------------------------------------------------------
    ...
    if not agent_list:
        ...
        c2.metric("Autonomy Level", row["autonomy_level"])
